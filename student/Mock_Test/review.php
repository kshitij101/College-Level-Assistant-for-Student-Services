...
$rs=mysqli_query($conn,"select * from mst_useranswer where sess_id='" . session_id() ."'") or die(mysqli_error($conn));
...
echo "<form name=myfm method=post action=review.php>";
	echo "<div class=panel>
					...
					";
echo "<table class=table> <tr> <td width=30>&nbsp;<td> <table class=table></td></tr>";
echo "<h1 class=page-title> Review Test Question</h1>";
$n=$_SESSION[qn]+1;
echo "<tr><td><h2 class=page-title><span >Que ".  $n .": $row[2]</style></span></h2></td></tr>";
echo "<tr><td class=".($row[7]==1?'tans':'style8').">$row[3]</td></tr>";
echo "<tr><td class=".($row[7]==2?'tans':'style8').">$row[4]</td></tr>";
echo "<tr><td class=".($row[7]==3?'tans':'style8').">$row[5]</td></tr>";
echo "<tr><td class=".($row[7]==4?'tans':'style8').">$row[6]</td></tr>";
if($_SESSION[qn]<mysqli_num_rows($rs)-1)
echo "<tr><td></br><button type=submit class=btn btn-primary btn-lg btn-block name=submit type=submit id=submit value='Next Question'>Next Question</button></td></tr></form>";
else
echo "<tr><td></br><button type=submit class=btn btn-primary btn-lg btn-block name=submit type=submit id=submit value='Finish'>Finish</button></td></tr></form>";
