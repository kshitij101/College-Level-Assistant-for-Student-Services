			...
			<div class="sidebar-scroll">
				<nav>
					<ul class="nav">
						...
						<li><a href="#" class="active"><i class="fa fa-book"></i> <span>Notes</span></a></li>
						...
						<li><a href="Mock_Test/mock_Stud.php" ><i class="fa fa-graduation-cap"></i> <span>Mock Test</span></a></li>
 ...
 <table class="table">
  <thead>
   <tr>
	...
	{
			$q="SELECT * FROM presentation WHERE subject='$row->course_code' ORDER BY subject desc limit $k,$dis";
			$ros=mysqli_query($conn,$q);
			
			while($row1=mysqli_fetch_array($ros))
			{
				echo '<tr>';
				echo '<td>' . $row->course_name;
				echo '<td>' .$row1['topic'];
				echo "<td><a title='Click here to download in file.' 
					 href='download.php?id={$row1['file']}'><button class='btn btn-primary'><i class='fa fa-download'></i> </button></a>"; 
				echo '</tr>';
			...
			for($i=1;$i<$total_page;$i++)
			{
				...
				{
				echo '<a href="display.php?page='.$i.'"> <button type="button" class="btn btn-primary" value="'.$i.'">'.$i.'</button></a> ';
			...
			if($page_cur<$total_page)
			{
				...
				echo '<a href="display.php?page='.($page_cur+1).'"><button type="button" class="btn btn-primary" value="next">next</button></a>';
