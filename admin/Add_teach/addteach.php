<?php
    ...
    $result = mysqli_query($conn,"SELECT * FROM teach_info");
...
<form method="POST" action="addt.php" class="form-control">
<?php
    foreach($fields as $field)
    {
        echo "<div class='col-md-6'><label for='$field->name'>".$field->name."</label></div>";
  
        echo "<div class='col-md-6'><input id='$field->name' name='$field->name' type='text'></div>";
       
    }
?>
   <div class="col-md-6"><label for="password">Password:</label></div>
    <div class="col-md-6"><input id="password" name="password" type="password"/></div>
    <div class="col-md-6"><label for="prof-pic">Select Profile Picture</label></div>
    <div class="col-md-6"><input type="file" name="fileToUpload" id="prof-pic"></div>
	
    <button class="btn btn-default" type="submit" value="Add teacher">Add teacher</button>
</form>
</div>
</div>
             </div>
